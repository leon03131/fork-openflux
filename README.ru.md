...
1. **onlyoffice** — пакеты через курсорные сообщения текущего редактора Яндекс Документов (OnlyOffice);
2. **mail** — пакеты через курсорные сообщения документов Mail.ru Облака (Р7-Офис);
3. **direct** — эталонный TCP carrier (быстрый, нужен открытый порт на ноде);
4. **oneme** — пакеты, замаскированные под ICE-кандидаты, через сигналинг MAX (экспериментальный);
5. **yandex** — legacy-редактор Яндекс Документов (deprecated: провайдер мигрировал).
