...
Клиентская часть запускает SOCKS5-прокси, выходная нода декапсулирует
и пересылает пакеты в пункт назначения.

Транспорты (carrier'ы):

| Carrier | Статус | Примечания |
|---|---|---|
| **direct** | ✅ стабильный | Чистый TCP; самый быстрый; на ноде нужен открытый порт |
| **onlyoffice** | ✅ проверен живьём | Текущие Яндекс Документы (редактор OnlyOffice), публичная ссылка с правами редактирования |
| **mail** | ✅ проверен живьём | Документы Mail.ru Облака (Р7-Офис), публичная ссылка с правами редактирования |
| **oneme** | 🧪 экспериментальный | Сигналинг звонков MAX (инъекция ICE-кандидатов) |
| **yandex** | ⚠️ deprecated | Legacy-редактор Яндекс Документов — провайдер его убрал |
...
```bash
go mod tidy
go build -o openflux .    # openflux.exe на Windows
```
...
- **legacy** (`--mode legacy`) — исходный gVisor packet-туннель с
  raw-сокетами (ноде нужен root; **только Linux/macOS** — Windows
  ограничивает raw-сокеты на уровне ОС; DNS в legacy резолвится локально).

v2 **требует** общий секрет (`--psk` / `OPENFLUX_PSK`) — случайный
32-байтовый ключ в base64 или hex. Сгенерируйте: `openssl rand -base64 32`.
Пароли-фразы отклоняются (подвержены офлайн-брутфорсу). Флаг
`--insecure` отключает шифрование — только для локального тестирования.
