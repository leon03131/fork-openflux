...
- `transport/reliable/` — reliability-адаптер поверх lossy carrier'а: state-replication (snapshot всех unacked-фреймов + cumulative ackThrough, epoch на поколение, `NewGeneration()` при rebuild сессии), строгий FIFO наверх. Формат OFR2: channelID (HKDF из PSK, изоляция пар на общем документе) + HMAC-SHA256(16) метаданных + CRC32. Bounds: 256 кадров / min(2 MiB, бюджет snapshot'а в inner.MaxPayload) unacked — wire-учёт (12B+payload на кадр), backpressure в Send; retransmit ~500мс. Send-контракт: принятый в окно кадр не возвращает inner-ошибки наружу (retransmit лечит), IsConnected игнорирует transient-reconnect carrier'а. Детали — в doc-comment пакета.
